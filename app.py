...
def calculate_shoe_data(casing, survey, temp, well_info, fitlot):
    ...
    for _, row in casing.dropna(subset=["String_ID","Shoe_MD_ft"]).iterrows():
        ...
        top_tvd = interp_by_md(survey, float(row["Top_MD_ft"]), "TVD_ft") if pd.notna(row.get("Top_MD_ft")) else np.nan
...
def draw_wbs(well, casing, shoes, geocalc, fitlot=None):
    """
    Professional three-column WBS renderer:
      left   = geological tops / formation intervals
      center = nested casing + cement
      right  = casing shoe engineering callouts
    Vertical scale is MD for operational readability.
    """
    ...
    if not shoes.empty and "Shoe_MD_ft" in shoes:
        all_depths += shoes["Shoe_MD_ft"].dropna().astype(float).tolist()
    ...
    ax.text((geo_left + right_edge)/2, -0.055*max_depth, "WELLBORE DIAGRAM",
            ha="center", va="center", fontsize=17, fontweight="bold")
    ax.text((geo_left + right_edge)/2, -0.022*max_depth, well_name,
            ha="center", va="center", fontsize=13, fontweight="bold")
    if prospect and prospect != "nan":
        ax.text((geo_left + right_edge)/2, 0.002*max_depth, prospect,
    ...
    summary = (
        ...
        f"RKB: {rkb:,.0f} ft\n"
        f"Mudline: {mudline_md:,.0f} ft MD/TVD ref\n"
        f"Planned TD: {td_md:,.0f} ft MD"
    )
    ax.text(right_edge-0.1, 0.02*max_depth, summary, ha="right", va="top",
            fontsize=8.5,
            bbox=dict(boxstyle="round,pad=0.4", facecolor="white", edgecolor="0.4"))

    # Column headers
    y_head = 0.07*max_depth
    ax.text((geo_left+geo_right)/2, y_head, "GEOLOGICAL TOPS", ha="center",
            va="center", fontsize=9, fontweight="bold")
    ax.text(well_center, y_head, "CASING / CEMENT", ha="center",
            va="center", fontsize=9, fontweight="bold")
    ax.text((callout_x+right_edge)/2, y_head, "SHOE DATA", ha="center",
            va="center", fontsize=9, fontweight="bold")

    # Mudline and water interval
    ax.hlines(mudline_md, geo_left, right_edge, linewidth=1.2)
    ax.text(geo_left, mudline_md-0.008*max_depth, f"Mudline {mudline_md:,.0f} ft",
            fontsize=8, va="bottom")

    # subtle column boundaries
    ax.vlines([geo_right, 2.7], 0.085*max_depth, max_depth*1.015, linewidth=0.7, alpha=0.5)

    # Formation shading between geological tops
    g = geocalc.dropna(subset=["MD_ft"]).sort_values("MD_ft").copy()
    if not g.empty:
        depths = g["MD_ft"].astype(float).tolist()
        names = g["Formation_Name"].astype(str).tolist()
        for i, (_, row) in enumerate(g.iterrows()):
            top = float(row["MD_ft"])
            bot = float(g.iloc[i+1]["MD_ft"]) if i < len(g)-1 else max_depth
            category = str(row.get("Category","")).lower()
            # no explicit colors: use grayscale hatch / alpha instead
            alpha = 0.06 if i % 2 == 0 else 0.11
            ax.add_patch(Rectangle((geo_left, top), geo_right-geo_left, max(bot-top, 1),
                                   facecolor="0.5", alpha=alpha, edgecolor="none"))
            ax.hlines(top, geo_left, 2.5, linestyles="dashed", linewidth=0.8)
            label = (
                f'{row["Formation_Name"]}\n'
                f'MD {row["MD_ft"]:,.0f} ft | TVD {row["TVD_ft"]:,.0f} ft'
            )
            if pd.notna(row.get("Temperature_F")):
                label += f'\nT {row["Temperature_F"]:.0f}°F'
            ax.text(geo_left+0.08, top+0.012*max_depth, label, fontsize=7.3, va="top")

    # Nested casing strings
    ...
    n = max(len(c), 1)
    widths = np.linspace(2.35, 0.9, n)

    # Cement background first
    ...
    for width, (_, row) in zip(widths, c.iterrows()):
        ...
        ax.hlines(shoe, -width/2, width/2, linewidth=2.2)

        # label inside string if space permits
        midy = (top + shoe)/2
        name = str(row["String_Name"])
        size_txt = f'{od:g}"' if pd.notna(od) else ""
        ax.text(well_center, midy, f"{name}\n{size_txt}", ha="center",
                va="center", fontsize=7.2, rotation=90 if (shoe-top) > 3500 else 0)

        # TOC indicator
        if pd.notna(row.get("TOC_MD_ft")):
            toc = float(row["TOC_MD_ft"])
            ax.hlines(toc, -width/2-0.15, width/2+0.15, linewidth=0.8, linestyles=":")
            ax.text(width/2+0.22, toc, f"TOC {toc:,.0f}", fontsize=6.7, va="center")

    # Shoe callouts
    for idx, row in shoes.sort_values("Shoe_MD_ft").iterrows():
        y = float(row["Shoe_MD_ft"])
        # leader line
        ax.hlines(y, 1.25, callout_x-0.1, linewidth=0.8)
        parts = [
            f'{row["String_Name"]}',
            f'MD {row["Shoe_MD_ft"]:,.0f} | TVD {row["Shoe_TVD_ft"]:,.0f}',
            f'Inc @ shoe {row["Inc_at_Shoe_deg"]:.1f}° | Max {row["Max_Inc_to_Shoe_deg"]:.1f}°',
        ]
        if pd.notna(row.get("Temperature_F")):
        ...
        txt = "\n".join(parts)
        ax.text(callout_x, y, txt, fontsize=7.1, va="center",
                bbox=dict(boxstyle="round,pad=0.28", facecolor="white", edgecolor="0.55"))

    # TD marker
    ax.hlines(td_md, -1.0, 1.0, linewidth=1.8)
    ax.text(1.15, td_md, f"TD {td_md:,.0f} ft MD", va="center", fontsize=8, fontweight="bold")

    # Footer notes
    footer_y = max_depth*1.025
    ax.text(geo_left, footer_y,
            "Auto-generated schematic. Depth display is MD; calculated TVD/TVDSS values are derived from the directional survey.",
            fontsize=7, va="top")

    ax.set_xlim(geo_left-0.3, right_edge+0.2)
    ax.set_ylim(max_depth*1.06, -0.075*max_depth)
...
with tab1:
    ...
    with c2:
        st.subheader("Calculated shoe data")
