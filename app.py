...
def draw_wbs(well, casing, shoes, geocalc, fitlot=None):
    """
    Clean column-based WBS:
    LEFT: Geological Tops | Pore Pressure
    CENTER: Wellbore schematic
    RIGHT: Hole/Mud | FIT/LOT | Casing Specs
    """
    ...
    ax.text(1.5, -0.065 * max_depth, "WELLBORE DIAGRAM",
            ha="center", fontsize=17, fontweight="bold")
    ax.text(1.5, -0.035 * max_depth, well_name,
            ha="center", fontsize=13, fontweight="bold")
    if prospect and prospect != "nan":
        ax.text(1.5, -0.012 * max_depth, prospect,
                ha="center", fontsize=10)

    # Column headers
    header_y = 0.055 * max_depth
    headers = [
    ...
    for x, txt in headers:
        ax.text(x, header_y, txt, ha="center", va="center",
                fontsize=9, fontweight="bold")

    # Vertical separators only
    separators = [-5.7, -3.2, 1.7, 4.2, 6.1]
    for x in separators:
        ax.vlines(x, 0.07 * max_depth, max_depth * 1.015, linewidth=0.6, alpha=0.45)
    ...
    ax.text(right_edge, 0.005 * max_depth, summary,
            ha="right", va="top", fontsize=8.2)

    # Mudline marker - short local mark only
    ax.text(x_geo - 0.9, mudline_md, f"Mudline {mudline_md:,.0f} MD",
            va="center", fontsize=7.5)
    ax.plot([-0.8, 0.8], [mudline_md, mudline_md], linewidth=1.2)
    ...
    for _, row in g.iterrows():
        ...
        ax.text(x_geo, y, txt, ha="center", va="center", fontsize=7.1)

    # Prepare casing strings
    ...
    widths = np.linspace(2.25, 0.8, max(len(c), 1))

    # Draw cement and casing
    for width, (_, row) in zip(widths, c.iterrows()):
        ...
        ax.add_patch(Rectangle(
            (-ann_width / 2, max(top, toc)),
            ann_width,
            max(shoe - max(top, toc), 1),
            facecolor="0.8",
            alpha=0.35,
            edgecolor="none"
        ))
        ...
        # short shoe cap
        ax.plot([-width / 2, width / 2], [shoe, shoe], linewidth=1.8)

        # minimal wellbore labels
        od = row.get("Casing_OD_in", np.nan)
        label = f'{od:g}"' if pd.notna(od) else str(row["String_Name"])
        ax.text(0, (top + shoe) / 2, label,
                ha="center", va="center", fontsize=6.8)

    # Per-hole-section columns
    for _, row in shoes.sort_values("Shoe_MD_ft").iterrows():
        ...
        if pd.notna(ppmin) and pd.notna(ppmax):
            ...
            pp_text = f"{ppmax:.1f} ppg"
        else:
            pp_text = "-"
        ax.text(x_pp, mid, pp_text, ha="center", va="center", fontsize=7.2)
        ...
        # FIT / LOT column - positioned at shoe
