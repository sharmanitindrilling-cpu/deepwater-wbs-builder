...
def draw_wbs(well, casing, shoes, geocalc, fitlot=None):
    """
    Stage 8:
    - move Shoe / Top(TOL) / TOC detail block to the LEFT of the wellbore
    - keep mudline-based scale and MD/TVD display
    """
    ...
    separators = [-7.5, -4.8, -1.2, 2.2, 5.1, 7.0]
    for x in separators:
    ...
    for (_, row), y_text in zip(g.iterrows(), geo_adj):
        ...
        ax.plot([-7.65, -7.5], [y_actual, y_actual], color="black", linewidth=0.8)
    ...
    detail_actual = [float(r["Shoe_MD_ft"]) + 250 for _, r in c.iterrows()]
    detail_y = adjust_positions(detail_actual, min_sep=950, ymin=header_y + 400, ymax=max_depth * 0.95)
