...
def draw_wbs(well, casing, shoes, geocalc, fitlot=None):
    ...
    def draw_tol_symbol(ax, x_left, x_right, y, h=300, color='black'):
        ...
        # Inner black fill bars start directly at the casing walls so there is no white gap.
        ax.add_patch(Rectangle((x_left, y - h * 0.78), inner_w, h * 0.78, facecolor=color, edgecolor=color, linewidth=0))
        ax.add_patch(Rectangle((x_right - inner_w, y - h * 0.78), inner_w, h * 0.78, facecolor=color, edgecolor=color, linewidth=0))
        shelf_y = y - h * 0.58
