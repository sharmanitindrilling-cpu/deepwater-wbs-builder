...
def draw_wbs(well, casing, shoes, geocalc, fitlot=None):
    ...
    for width, (_, row) in zip(widths, c.iterrows()):
        ...
        if shoe > cement_top:
            ax.add_patch(Rectangle((x_well - annulus_outer / 2, cement_top), ann_gap, shoe - cement_top,
                                   facecolor='dimgray', alpha=0.70, edgecolor='none'))
            ax.add_patch(Rectangle((x_well + width / 2, cement_top), ann_gap, shoe - cement_top,
                                   facecolor='dimgray', alpha=0.70, edgecolor='none'))
...
def draw_3d_directional_wbs(
    ...
    """3D directional WBS using Easting, Northing and TVD with user toggles."""
    ...
    # Casing strings and shoe markers/labels.
    csort = shoes.sort_values('Shoe_MD_ft').reset_index(drop=True).copy()
    if not csort.empty:
    ...
    # Geological tops positioned on the 3D survey.
...
with tab7:
    st.subheader("3D Directional WBS Controls")

    t1, t2, t3, t4 = st.columns(4)
    with t1:
        show_trajectory = st.toggle("Survey trajectory", value=True, key="3d_trajectory")
        show_casing = st.toggle("Casing strings", value=True, key="3d_casing")
    with t2:
        show_shoes = st.toggle("Casing shoes", value=True, key="3d_shoes")
    ...
    with t4:
        show_coordinates = st.toggle("Coordinate box", value=True, key="3d_coordinates")
