...
def draw_wbs(well, casing, shoes, geocalc, fitlot=None):
    ...
    def draw_tol_symbol(ax, x_left, x_right, y, h=300, color='black'):
        """
        Top-of-liner hanger styled to match the reference snapshot:
        a solid black hanger/shoulder connects the outer casing wall to the liner wall
        on both sides, with the liner continuing downward.
        """
        outer_wall_w = 0.10
        hanger_h = h * 0.34
        hanger_top = y - h * 0.72

        # Outer casing-side black shoulder strips.
        ax.add_patch(Rectangle((x_left - outer_wall_w, hanger_top), outer_wall_w, hanger_h,
                               facecolor=color, edgecolor=color, linewidth=0))
        ax.add_patch(Rectangle((x_right, hanger_top), outer_wall_w, hanger_h,
                               facecolor=color, edgecolor=color, linewidth=0))

        # Solid hanger blocks extending inward from the outer casing to the liner wall.
        hanger_span = 0.22
        ax.add_patch(Rectangle((x_left - 0.01, hanger_top), hanger_span, hanger_h,
                               facecolor=color, edgecolor=color, linewidth=0))
        ax.add_patch(Rectangle((x_right - hanger_span + 0.01, hanger_top), hanger_span, hanger_h,
                               facecolor=color, edgecolor=color, linewidth=0))

        # Small rounded-looking cap effect using an overlapping upper block.
        cap_h = hanger_h * 0.32
        ax.add_patch(Rectangle((x_left + 0.03, hanger_top - cap_h * 0.35), hanger_span * 0.72, cap_h,
                               facecolor=color, edgecolor=color, linewidth=0))
        ax.add_patch(Rectangle((x_right - hanger_span * 0.72 - 0.03, hanger_top - cap_h * 0.35),
                               hanger_span * 0.72, cap_h, facecolor=color, edgecolor=color, linewidth=0))

        # Short liner-side vertical black continuation below the hanger.
        liner_bar_w = 0.055
        lower_h = h * 0.40
        ax.add_patch(Rectangle((x_left + hanger_span - liner_bar_w, hanger_top + hanger_h),
                               liner_bar_w, lower_h, facecolor=color, edgecolor=color, linewidth=0))
        ax.add_patch(Rectangle((x_right - hanger_span, hanger_top + hanger_h),
                               liner_bar_w, lower_h, facecolor=color, edgecolor=color, linewidth=0))
    ...
    for idx, row in shoes_sorted.iterrows():
        ...
        ax.text(x_hole, section_y, '\n'.join(hole_lines), ha='center', va='top', fontsize=7.8)
