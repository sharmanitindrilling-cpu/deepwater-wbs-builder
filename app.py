...
def draw_wbs(well, casing, shoes, geocalc, fitlot=None):
    ...
    # Begin the visible schematic 1000 ft above mudline
    visible_top = max(0.0, mudline_md - 1000.0)
    ...
    # Put column headers just above mudline
    header_y = mudline_md - 220
    ...
    for width, (_, row) in zip(widths, c.iterrows()):
        ...
        ax.add_patch(Rectangle((x_well - width / 2, top), width, max(shoe - top, 1), fill=False, linewidth=1.9))
